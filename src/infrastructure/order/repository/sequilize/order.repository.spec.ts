...
describe("Order repository test", () => {
  ...
  it("should must update the sum of the order items", async () => {
    const customerRepository = new CustomerRepository();
    const customer = new Customer("123", "Customer 1");
    const address = new Address("Street 1", 1, "Zipcode 1", "City 1");
    customer.changeAddress(address);
    ...
    expect(order.total()).toEqual(220);
    expect(orderModelUpdate.total).toEqual(order.total());
  });
  ...
  it("should find an order", async () => {
    ...
    const orderEntity = new Order("1", "1", [ordemItem]);
  ...
  it("should find all order", async () => {
    ...
    const orderItem = order[0].items[0];
